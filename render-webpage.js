        ...
        page.includeJs('https://cdn.jsdelivr.net/jquery/1.12.3/jquery.min.js', function() {
            page.evaluate(function() {
                ...
                if (window.location.href.indexOf('/report') > -1) {
                    ...
                    // Don't know why yet but this is what's been causing the PDF renderer to use the screen stylesheet instead of print
                    // -Roscoe
                    // $main.prop('media', 'print');
                    ...
                    //get the longest panel out of all panels being displayed
                    var maxPanelWidth = Math.max.apply(Math, $panels.map(function() {
                        return $(this).width();
                    }).get());
                    ...
                    for (var i = Highcharts.charts.length - 1; i >= 0; i--) {
                        ...
                        $container.width(maxPanelWidth);
                        $renderTo.width(maxPanelWidth);

                        if ($panel.find('.highcharts-container').length > 1) {
                            chart.setSize(maxPanelWidth - 15, chartHeight);
                        } else {
                            chart.setSize(maxPanelWidth - 15, $panelbody.height());
                        }
                    ...
                    var currentHeight = pageOneInitialHeight;
                    ...
                    $.each($panels, function(index, val) {
                        ...
                        // var panelHeight = $panel.height(); //ignores margins

                        if ((currentHeight + panelHeight) > pdfHeight) {
                            // $panel.append('<h4>New page needed! </h4>');//, Panel outerHeight: ' + panelHeight + ' Panel height ' + $panel.height() + ', Space: ' + (currentHeight + panelHeight) + '</h4>');
                            $panel.css({
                            ...
                            // $panel.append('<h4>No new page needed! </h4>');// $panel.prepend('<h4>No new page needed!, Panel outerHeight: ' + panelHeight + ' Panel height ' + $panel.height() + ', Space: ' + (currentHeight + panelHeight) + '</h4>');

                            //content height has increased
                            currentHeight = currentHeight + panelHeight;
                        }
                    });
                }
            });
